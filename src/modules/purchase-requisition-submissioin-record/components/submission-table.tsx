...
const PurchaseRequisitionSubmissionTable: React.FC<IPurchaseRequisitionSubmissionTableProps> = (props) => {
  const PURCHASE_REQUISITION_SUBMISSION_TABLE_COLUMN: ColumnsType<IPurchaseRequisitionRequestItem> = [
    ...
    },
    {
      ...
      key: 'vendor',
      align: 'center',
      render: (text: string, record: IPurchaseRequisitionRequestItem) => <span>{text}</span>,
    },
    {
  ...
  if (props.currentSubmissionRecord && props.currentSubmissionRecord != null) {
    ...
    return (
      <>
        <Table className="my-4" dataSource={submissionItems} columns={PURCHASE_REQUISITION_SUBMISSION_TABLE_COLUMN} rowKey="id" scroll={{ y: 370 }} pagination={TABLE_PAGINATION_CONFIG}></Table>
      </>
    );
  } else {
    return (
